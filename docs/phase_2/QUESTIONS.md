...
Если поле `Ответ` заполнено, оно главнее поля `Текущее предположение`. Нормализованные решения для реализации перенесены в `docs/phase_2/DECISIONS.md`.
...
- Ответ: python -m pytest из venv, зависимости в requirements.txt
...
- Ответ: рабочие директории запусков
...
- Ответ: reject, всё должно быть явно
...
- Ответ: file path, YAML path, expected field/type, preview фактического значения.
...
- Ответ: нижний цикл рассказывает резльтат по заранее специфицированному формату верхнему
...
- Ответ: количество запросов, токены или то, сколько денег возвращает провайдер
...
- Ответ: resolved path, с будущим jail-mount
...
- Ответ: allowlist + blacklist по регуляркам
...
- Ответ: Это отдельные тулы с описаными ограничениями
...
- Ответ: model, messages, config, tools
...
- Ответ: отдельная task типа model, внутри которой делается верификация. можно сделать шаблон verifier
...
- Ответ: status с доказательством + ссылкой на аудит лог
...
- Ответ: развесистая структура папок
...
- Ответ: структуру циклов/тасок, лог исполнения с возможностью фильтровать по циклам, таскам, смотреть на то, что в конкретном цикле / таске / связи / туле / ... происходило
...
- Ответ: static HTML с ленивой подгрузкой из развесистой структуры
...
- Ответ: ещё на то что кончился бюджет, на то, что модель доретраилась, короч на каждый элемент success сценарий и несколько плохих сценариев из разных мест
...
- Ответ: выше написал, что на каждый элемент
...
- Ответ: давай несколько файлов, чтобы не совсем было просто, но без внешних зависимостей + agents.md внутри и architecture.md
...
- Ответ: несколько раз прогонять цикл, в этом суть, и stub тесты тоже на это сделать
...
- Ответ: Проверка, что всё, что описано в таске, реализовано
...
- Ответ: Нам важно в первую очередь прототип, proof of work. не надо вылизывать архитектуру, гоняться за скоростью, надо чтобы было понятно, просто, модифицируемо, поддерживать локальность, комментарии прям в коде, docstring-и, в которых ответы на вопросы «почему мы так сделали»
